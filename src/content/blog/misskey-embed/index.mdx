...
こんな感じで、ブログに張り付けられるパーツのことです。ちゃんと添付ファイルも表示されるし、カスタム絵文字やMFM（けたたましく動き回るアニメーションとかがつくれる独自記法）にも対応していて、非常に見栄えがいい。  
...
共通化できる部分はどしどしコンポーネントにしましょう、ってAstroのチュートリアルにも書いてあったような気がしますし、Copilot先生に聞きながら実際にやってみました。
...
つまり、macとWindows機を行き来しても、StackBlitzで書いても、同じ方法で簡単に補完できるようになりました。
